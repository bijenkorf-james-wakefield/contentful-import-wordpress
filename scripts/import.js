...
class Importer {
  ...
  async createContentEntryWithId (typeID, entryId, entry) {
    ...
    try {
      ...
      console.log('error on entry', entry, err)
      return new Promise((resolve, reject) => {
        setTimeout(() => {
          reject(err)
        }, 2000)
  ...
  // Steps to tidy up HTML
  // 1. Remove inline styles from html string
  // 2. Remove tabs
  // 3. Remove double whitespace
  // 4. Remove double line breaks
  // 4. Add paragraph tags
  // 5. Parse HTML
  // 6. Create RichTextNodes
  mapPost (input) {
                          ...
                          util.removeDoubleSpaces(
                            util.removeUnwantedHtml(
                              util.removeNonBreakingSpaces(
                                util.removeInlineStyles(
                                  input['content:encoded'][0]
                                )
                              )
                            )
                          )
  ...
  analyseXml () {
    ...
    console.log(`found: ${ this.getItemByType('attachment').length } attachments`)
  ...
  async importWordpressData (exportFile) {
    ...
    let posts = this.getItemByType('post').map(post => {
    ...
    let filteredPagesToSave = mergedPages.filter((item, i) => {
      return [9337, 337].includes(parseInt(item.fields.id, 10))
    })
